...
class Maze:
    ...
    def get_nearest_waypoints(self, cur_waypoint, cur_shape):
        ...
        for waypoint in valid_waypoints:
            ...
            if self.is_valid_move(start, end):
                ...
                distances_map[waypoint] = min(d, distances_map.get(waypoint, float('inf')))

        distances_map_sorted = dict(sorted(distances_map.items(), key=lambda item: item[1]))

        for key in distances_map_sorted.keys():
            if len(nearest_neighbors) == self.k:
                break
            else:
                nearest_neighbors.append(key)
    ...
    def is_valid_move(self, start, end):
        ...
        if (start[2] < 0 or start[2] > 2) or (end[2] < 0 or end[2] > 2) or (abs(start[2] - end[2]) >= 2):
            return False

        start_alien = self.create_new_alien(*start)
        end_alien = self.create_new_alien(*end)

        if does_alien_touch_wall(start_alien, self.walls) or does_alien_touch_wall(end_alien, self.walls):
            return False
