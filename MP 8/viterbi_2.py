...
def training(sentences):
    ...
    for key in hapax.keys():
        hapax[key] = hapax[key] / total_hapex
    ...
    for tag in emit_prob.keys():
        ...
        current_alpha = alpha * hapax.get(tag, float(1.0/total_hapex)) 

        laplace_denom = (nt + (current_alpha * (Vt + 1)))
        
        for word in emit_prob[tag]:
            emit_prob[tag][word] = float((emit_prob[tag][word] + current_alpha) / laplace_denom)

        emit_prob[tag]["UNKNOWN"] = float(current_alpha / laplace_denom)
...
def viterbi_stepforward(i, word, prev_prob, prev_predict_tag_seq, emit_prob, trans_prob):
    ...
    if i == 0:
        for tag in emit_prob.keys():
            ...
            pE_unknown = emit_prob[tag].get("UNKNOWN", emit_epsilon)
        ...
        for tag_b in emit_prob.keys():
            ...
            for tag_a in emit_prob.keys():
                ...
                pE_unknown = emit_prob[tag_b].get("UNKNOWN", emit_epsilon)
